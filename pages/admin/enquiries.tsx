...
const Enquiries = () => {
  ...
  const inputClasses =
    "h-2.5 px-0.75 relative w-full flex rounded font-heading text-md items-center";
  const actionClasses =
    "w-full bg-primary-base hover:bg-primary-hover text-center justify-center relative inline-flex rounded font-heading text-md items-center text-white h-2.5 px-1 self-end";
  ...
  const dateRange = (
    <div className="flex space-x-1">
      ...
      <CSVLink
        ...
        data={csvData}
        filename={`${activeTab}-from-${dateFrom}--to-${dateTo}.csv`}
      >
        Download as CSV
      </CSVLink>
    </div>
  ...
  return (
    <AdminLayout title="Form submissions" action={dateRange}>
      <div className="m-1 mb-0 flex space-x-0.5">
        <button
          className={`h-1.75 flex text-xs focus:outline-none ${
            activeTab == "estimate-requests"
              ? "text-white bg-primary-base"
              : "text-black bg-neutral-4"
          } items-center justify-center px-0.75 py-0.5 rounded-full`}
          onClick={() => handleTab("estimate-requests")}
        >
          Estimate requests
        </button>
        <button
          className={`h-1.75 flex text-xs focus:outline-none ${
            activeTab == "download-requests"
              ? "text-white bg-primary-base"
              : "text-black bg-neutral-4"
          } items-center justify-center px-0.75 py-0.5 rounded-full`}
          onClick={() => handleTab("download-requests")}
        >
          Brochure requests
        </button>
        <button
          ...
          onClick={() => handleTab("apprentice-requests")}
        >
          Apprenticeship
        </button>
      </div>
      ...
      <table className="table-fixed w-full">
        <thead>
          <tr>
            ...
            {activeTab == "apprentice-requests" && (
              <th className="text-left border-b border-gray-200 p-1">
                Address
              </th>
            )}
            {(activeTab == "apprentice-requests" ||
            ...
            {activeTab == "estimate-requests" && (
              <th className="text-left border-b border-gray-200 p-1">Files</th>
            )}
          </tr>
        </thead>
        <tbody>
          {enquires
            ? enquires?.map((enquiry, i) => (
                <tr key={i} className="align-top">
                  <td className="border-b border-gray-200 p-1 text-sm">
                    {enquiry.prettyDate ? (
                      enquiry.prettyDate
                    ) : (
                  ...
                  {activeTab == "estimate-requests" && (
                    <td className="border-b border-gray-200 p-1 text-sm space-y-1">
                      <DownloadButton files={enquiry.files} />
                    </td>
                  )}
                </tr>
              ))
...
const DownloadButton = ({ files }) => {
  ...
  return (
    <>
      {files && (
        <button onClick={() => setOpenDownloads(!openDownloads)}>
